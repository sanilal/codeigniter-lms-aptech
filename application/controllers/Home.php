...
class Home extends CI_Controller
{
    ...
    public function about()
    {
        ...
        $page_data['page_title'] = site_phrase('about');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

   /*  function _set_fields_1(){
       $this->form_data = new stdClass;
        $this->form_data->inputName = '';
        $this->form_data->inputEmail = '';
        $this->form_data->inputPhone = '';
        $this->form_data->inputCompanyName = '';
        $this->form_data->inputSubject = '';
        $this->form_data->inputLocation = '';
        $this->form_data->inputMessage = ''; 
    
       $this->form_data->footerInputName = '';
        $this->form_data->footerInputEmail = '';
        $this->form_data->footerInputPhone = '';
        $this->form_data->footerInputMessage = ''; 
    
        
    } */
    
    function _set_rules_1(){
        ...
        $this->form_validation->set_rules('inputCompany', 'inputCompany', 'required');
        $this->form_validation->set_rules('inputSubject', 'inputSubject', 'required');
        $this->form_validation->set_rules('inputLocation', 'inputLocation', 'required');
    ...
    function contact(){
		...
		$email_template =' <table width="80%" style="border-collapse: collapse;border-spacing: 0;box-sizing: border-box;">
                          ...
                          <tr><td style="border:1px solid #587C08;padding:6px;font-family:Arial, Helvetica, sans-serif;color:#ffffff;font-size:14px;background: #8ABE18;">Company Name</td><td style="border:1px solid #9E9898;padding:6px;font-family:Arial, Helvetica, sans-serif;color:#544747;font-size:14px;background: #ffffff;">'.$this->input->post('inputCompany').'</td></tr>

                          <tr><td style="border:1px solid #587C08;padding:6px;font-family:Arial, Helvetica, sans-serif;color:#ffffff;font-size:14px;background: #8ABE18;">Subject</td><td style="border:1px solid #9E9898;padding:6px;font-family:Arial, Helvetica, sans-serif;color:#544747;font-size:14px;background: #ffffff;">'.$this->input->post('inputSubject').'</td></tr>
                          
                          <tr><td style="border:1px solid #587C08;padding:6px;font-family:Arial, Helvetica, sans-serif;color:#ffffff;font-size:14px;background: #8ABE18;">Location</td><td style="border:1px solid #9E9898;padding:6px;font-family:Arial, Helvetica, sans-serif;color:#544747;font-size:14px;background: #ffffff;">'.$this->input->post('inputLocation').'</td></tr>
						  
						  
						  <tr><td style="border:1px solid #587C08;padding:6px;font-family:Arial, Helvetica, sans-serif;color:#ffffff;font-size:14px;background: #8ABE18;">Message</td><td style="border:1px solid #9E9898;padding:6px;font-family:Arial, Helvetica, sans-serif;color:#544747;font-size:14px;background: #ffffff;">'.$this->input->post('inputMessage').'</td></tr>
						</table>';
			            // save data
							//local
                          ...
                          $config['wordwrap']=TRUE;
							...
							$e_subject 		= ' Online Enquiry - Mail from '.$this->input->post('inputName').'';
    ...
    function request_quote(){
        ...
        $data['message'] = "<div class='alert alert-success'>
         ...
         // Extract the URI segments
    $uri_segments = explode('/', uri_string());

    // Set default page_name
    $data['page_name'] = 'contact';

    // Check if the URI has segments
    if (!empty($uri_segments)) {
        // Use the first segment as the page_name
        $data['page_name'] = $uri_segments[0];
    }
       
		$data['metatitle'] = 'Contact Us';
                $data['metadescription'] = 'Contact Aptech.';
		// load view
     //   $data['page_name'] = "contact"; // or dynamically set based on the current page
        $data['page_title'] = site_phrase('contact');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $data);
	}
    ...
    function _set_rules_3(){
        ...
        $this->form_validation->set_rules('inputCourseTitle', 'inputCourseTitle', 'required');
        $this->form_validation->set_rules('inputMessage', 'inputMessage', 'required');
    ...
    function course_contact(){
		...
		$email_template =' <table width="80%" style="border-collapse: collapse;border-spacing: 0;box-sizing: border-box;">
						  <tr>
						  <td height="52" colspan="2">
						  <img src="'.base_url('frontend/img/logo.png').'" alt="Aptech">
						  </td>
						  </tr>
						  <tr>
						  <td height="52" colspan="2">
			            ...
			            // save data
							...
							$config['wordwrap']=TRUE;
							$config['validate']=TRUE;
							$from_address 	= "aptechcomputersdubai@gmail.com";
							$to_address 	= "sanilal.iconcept@gmail.com";
  ...
  /*   public function contact()
    ...
    public function whizkid()
    {
        $page_data['page_name'] = "whizkid";
        $page_data['page_title'] = site_phrase('whizkid');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function course($slug = "", $course_id = "")
    {
        ...
        $page_data['page_bg_color'] = "bg-gray";
        /* new start -------------------------------------------------------------------------------------------------------------*/
    ...
    public function instructors()
    {
        ...
        $page_data['page_title'] = site_phrase('instructors');
    ...
    public function submit_quiz_modal($from = "")
    {
        $submitted_quiz_info = array();
        $container = array();
        $course_id = $this->input->post('course_id');
