...
if (!function_exists('is_root_admin')) {
    function is_root_admin($admin_id = '')
    {
        ...
        $query=$CI->db->query('SELECT * FROM permissions where admin_id='.$admin_id);
        // shabab changed due num rows error
        //$CI->db->where('admin_id', $admin_id);
        //$get_admin_permissions = $CI->db->get('permissions');
        if ($query->num_rows() == 0) {
            return true;
...
/* if (!function_exists('is_super_admin')) {
    function is_super_admin($admin_id = '')
    {
        ...
        $CI->db->where('is_super_admin', 1);
        $get_admin_permissions = $CI->db->get('users')->num_rows();
        if ($get_admin_permissions == 0) {
            return true;
        } else {
            return false;
        }
...
if (!function_exists('get_frontend_settings1')) {
    function get_frontend_settings1($key = '')
    {
        $CI    = &get_instance();
       // $this->load->database('shared', TRUE);
       $default_db = $CI->load->database('default', TRUE);
        //$CI->load->database('default', TRUE);

        $default_db->where('key', $key);
        $result = $default_db->get('frontend_settings')->row()->value;
        return $result;
    }
}
