...
function candidateSession() 
{
    $CI = get_instance();
    $CI->load->library('session');
    //var_dump($CI->session->userdata('user_login')); die(); $field = '' && $field == ''
    if (!empty($CI->session->userdata('user_id')) ) {
        return $CI->session->userdata('user_id');
    } 
    /*else if (isset($CI->session->userdata('user_login')[$field])) {
        return $CI->session->userdata('user_login')[$field];
    }*/
}
...
function departmentThumb($image, $width = '', $height = '') {
    ...
    if ($width == '' && $image) {
        $image = base_url().'uploads/departments/'.$image;
        return $image;
    }
    $image = explode('.', $image);
...
function candidateThumb2($image, $width = '', $height = '') {
    if ($image) {
        $image = base_url().'uploads/user_image/'.$image.'.jpg';
    } else {
