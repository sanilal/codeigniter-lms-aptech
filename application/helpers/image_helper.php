...
class ImageManipulator
{
    ...
    {
        if ($width == 0) {
            $scalingFactor = $height / $this->height;
            $newImageWidth = round($this->width * $scalingFactor);
            $newImageHeight = $height;
        } elseif ($height == 0) {
            $scalingFactor = $width / $this->width;
            $newImageHeight = round($this->height * $scalingFactor);
            $newImageWidth = $width;
        } else {
            $newImageHeight = $height;
            $newImageWidth = $width;
        ...
        $temp = imagecreatetruecolor($newImageWidth, $newImageHeight);
        imagealphablending($temp, false);
        imagesavealpha($temp, true);
        $color = imagecolorallocatealpha($temp, 0, 0, 0, 127);
        imagefill($temp, 0, 0, $color);
        imagecopyresampled($temp, $this->image, 0, 0, 0, 0, $newImageWidth, $newImageHeight, $this->width, $this->height);
        return $this->_replace($temp);
    }
    ...
    public function save($fileName, $type = IMAGETYPE_JPEG)
    {
        ...
        //dd(array(IMAGETYPE_GIF, IMAGETYPE_PNG, IMAGETYPE_JPEG));
        ...
        try {
            switch ($this->type) {
                case IMAGETYPE_GIF  :
                    ...
                    break;
                case IMAGETYPE_JPEG:
                    if (!imagejpeg($this->image, $fileName, 75)) {
                        throw new RuntimeException;
                    }
                    break;
                case IMAGETYPE_PNG:
     ...
     * @return int
     */
    public function resizeAndCropVertical()
    {
        return $this->height;
    }
}