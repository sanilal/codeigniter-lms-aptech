...
class AdminJobFilterModel extends CI_Model
{
    ...
    public function getAll($active = true)
    {
        $this->db->select('
            job_filters.*,
            GROUP_CONCAT(
                DISTINCT(CONCAT(job_filter_values.title, ")=-=(", job_filter_values.job_filter_value_id))
    ...
    public function job_filtersList()
    {
        ...
        $this->db->select('
            job_filters.*,
            GROUP_CONCAT(DISTINCT(job_filter_values.title) SEPARATOR ", ") as filter_values
        ');
        if ($srh) {
    ...
    private function prepareDataForTable($job_filters)
    {
        ...
        foreach ($job_filters as $c) {
            ...
            $sorted[] = array(
                ...
                ($c['admin_filter'] == 1 ? site_phrase('yes') : site_phrase('no')),
                ($c['front_filter'] == 1 ? site_phrase('yes') : site_phrase('no')),
                ($c['front_home_filter'] == 1 ? site_phrase('yes') : site_phrase('no')),
                ($c['front_value'] == 1 ? site_phrase('yes') : site_phrase('no')),
                ($c['type'] == 'dropdown' ? site_phrase('dropdown') : site_phrase('checkbox')),
