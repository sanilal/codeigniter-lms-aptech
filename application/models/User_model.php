...
class User_model extends CI_Model
{
    ...
    // public function get_instructor($user_id = 0)
    // {
    //     if ($user_id > 0) {
    //         $this->db->where('id', $user_id);
    //     }
    ...
    public function add_user($is_company = false,$is_instructor = false, $is_admin = false)
    {
        $validity = $this->check_duplication('on_create', $this->input->post('email'));
        if ($validity == false) {
            $this->session->set_flashdata('error_message', get_phrase('email_duplication'));
        } else {
            $data['company_title'] = html_escape($this->input->post('company_title'));
            ...
            // IF THIS IS A USER THEN INSERT BLANK VALUE IN PERMISSION TABLE AS WELL ["job_actions","job_add","job","job_edit"]
            if ($is_admin || $is_company) {
                $permission_data['admin_id'] = $user_id;
                   if ($is_company) {
                $permission_data['permissions'] = json_encode(array('job_actions','job_add','job','job_edit','job_delete'));
                   }
                   else{
    ...
    {
        if ($id > 0) {
            return $this->db->get_where('users', array('id' => $id, 'is_company' => 1));
        } else {
            return $this->db->get_where('users', array('is_company' => 1));
        }
